...
## Assuming a positive uniform prior for mu 


## Defining the likelihood, it is the function that represents the probability of measung a given value of D before taking the measure 
...
N <- length(N_par)
mean_analytical <- sum(N_par + 1) / (N + 1)
variance_analytical <- (sum(N_par) + 1) / ((N+1)^2 * (N + 2))

mean_numerical <- sum(mu_values * posterior)
median_numerical <- quantile(mu_values, 0.5, probs = posterior)[[1]]
variance_numerical <- sum((mu_values - mean_numerical)^2 * posterior)
...
posterior_density <- function(mu, data) {
  shape_prior <- k
  rate_prior <- theta
...
prior_values <- prior_density(x, shape = k, rate = theta)
...
posterior_values <- sapply(mu_values, posterior_density, data = N_par)
...
N <- length(N_par)
mean_analytical <- (sum(N_par) + mu_prior) / (N + 1)
median_analytical <- qgamma(0.5, shape = k + sum(N_par, scale = theta + 1))
variance_analytical <- (median_analytical) / (N + 1)

mean_numerical <- sum(mu_values * posterior)
median_numerical <- quantile(mu_values, 0.5, probs = posterior_density_normalized)[[1]]
variance_numerical <- sum((mu_values - mean_numerical)^2 * posterior_density_normalized)
...
those also recorded by Det2, researcher $\textbf{A}$ obtaines
$n \ = \ 500$ and $r \ = \ 312$.
...
calculate_posterior_mean_variance <- function(delta, posterior_values, p_values) {
  ...
  posterior_mean <- sum((p_values * posterior_values) * delta)
...
# Le distribuzioni sono uguali perché le prior sono uguali (flat prior e beta prior)
```
